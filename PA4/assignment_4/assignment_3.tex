...
\begin{homeworkProblem}[Better Classification Accuracy]
	...
	\paragraph{}
	对于SVM的参数设定，如果设定的$C$值过大，那么模型对误分类的惩罚值过大，容易出现过拟合的现象，模型在验证集和测试集上的表现会很差。但是如果设定的值过小，则会出现特征不明确的现象，误分类比例较大，同样会造成表现差的现象。对于PCA提取特征数目的设定，如果设定的数目过低，那么提取出的特征无法准确描述该分类的特征，但是如果提取的数目过多，那么会造成对噪音的过滤不够完全，优化效果不够明显的现象。从最终测试得到的较优的参数设定以及测试结果来看，每个分类大致需要35个特征值就可以准确唯一描述，同时$C=100$的惩罚值设定并没有造成过拟合现象。
\end{homeworkProblem}
